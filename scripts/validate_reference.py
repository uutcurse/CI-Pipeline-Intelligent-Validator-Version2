...
# Join to get normalized_workflow_text
...
print("Normalizing all workflows to find matches...")
workflows['norm_text'] = workflows['parsed_workflow_json'].apply(lambda x: normalize_workflow(json.loads(x)))

df = pd.merge(df_target, workflows, left_on='normalized_workflow_text', right_on='norm_text', how='inner').head(20)

print(f"Found {len(df)} matching workflows by text.")
...
for _, row in df.iterrows():
    ...
    if pred['predicted_class'] == true_label:
        if np.isclose(pred['probabilities']['LOW'], p0) and \
           np.isclose(pred['probabilities']['MEDIUM'], p1) and \
           np.isclose(pred['probabilities']['HIGH'], p2):
           matches += 1
           
print(f"Match: {matches}/{len(df)}")
